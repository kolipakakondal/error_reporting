...
import static org.eclipse.epp.internal.logging.aeri.ui.Constants.SYSPROP_SKIP_REPORTS;
import static org.eclipse.epp.internal.logging.aeri.ui.LogMessages.HISTORY_START_FAILED;
import static org.eclipse.epp.internal.logging.aeri.ui.LogMessages.HISTORY_STOP_FAILED;
import static org.eclipse.epp.internal.logging.aeri.ui.LogMessages.REPORTING_ERROR;
import static org.eclipse.epp.internal.logging.aeri.ui.utils.ErrorReports.guessInvolvedPlugins;
import static org.eclipse.epp.internal.logging.aeri.ui.utils.ErrorReports.newErrorReport;
import static org.eclipse.epp.internal.logging.aeri.ui.utils.Logs.log;
...
public class LogListener implements ILogListener, IStartup {
    ...
    public void logging(final IStatus status, String nouse) {
        try {
            if (!isReportingAllowedInEnvironment() || !isErrorSeverity(status) || isWorkbenchClosing() || !isHistoryRunning()
                    || isQueueLimitReached()) {
                return;
            }
    ...
    private void sendList() {
        for (ErrorReport entry : queueRO) {
            ErrorReport report = ErrorReport.class.cast(entry);
            sendStatus(report);
        }
    }
    ...
    private boolean seenSameOrSimilarErrorBefore(final ErrorReport report) {
        ...
        return history.seenSimilar(report) // did we send a similar error
                                           // before?
                || history.seen(report); // did we send exactly this error
                                         // before?
    }
    ...
    protected void checkAndSendWithDialog(final ErrorReport report) {
        // run on UI-thread to ensure that the observable list is not modified
        // from another thread
        // and that the wizard is created on the UI-thread.
        Display.getDefault().asyncExec(new Runnable() {
            @Override
            public void run() {
                // we only permit one dialog. If already open, skip this step
                // and add directly to the queue.
                ...
                try {
                    ...
                    if (shell.isPresent()) {
                        ErrorReportDialog reportDialog = new ErrorReportDialog(shell.get(), history, settings, queueUI) {
                            ...
                            public boolean close() {
                                ...
                                if (close) {
                                    // when closing, release the lock to let
                                    // another thread reopen it.
                    ...
                    // if something goes wrong in the internals of eclipse and
                    // throws an exception (it did in the past):
                    sendDialogLock.release();
                }
            }
        });
    ...
    protected void sendStatus(final ErrorReport report) {
        // double safety. This is checked before elsewhere. But just to make
        // sure...
